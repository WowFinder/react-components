...
describe('ClassPicker', () => {
    it('should render the correct number of options', () => {
        render(<ClassPicker classes={[...mockedClasses]} allowEmpty={true} />);
    ...
    it('should update the selection when the user picks another entry', async () => {
        ...
        const parentSelect = rendered.container.querySelector(
            'select',
        ) as HTMLSelectElement;
        expect(getSelectedOption().value).toBe(initialSelection);
        const manualSelection = 'mocked-arcane-class';
        const emptyOption = getOptionByValue(rendered, '');
        const manualSelectionOption = getOptionByValue(
            rendered,
            manualSelection,
        );
        await act(async () => {
        ...
        await act(async () => {
            ...
            manualSelectionOption.dispatchEvent(
